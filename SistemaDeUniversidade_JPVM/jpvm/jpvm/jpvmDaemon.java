...
class jpvmExecTaskThread extends Thread {
	...
	jpvmExecTaskThread(jpvmEnvironment j, jpvmTaskId c, String a[], jpvmCreateWorkOrder o) {
		jpvm = j;
		client = c;
		args = a;
		order = o;
	...
	private boolean doExec() {
		try {
			...
			buf.pack(-1);
			try {
				jpvm.pvm_send(buf, client, jpvmDaemonMessageTag.jpvmdCreatedTask);
...
public class jpvmDaemon {
	...
	private static void SpawnTask(jpvmTaskId client, jpvmBuffer req) {
		...
		if (num == 0) {
			...
			try {
				...
				perror("SpawnTask, problem sending " + "to client " + client.toString());
			}
			return;
		}
		...
		for (int i = 0; i < num; i++) {
			...
			if (target.tid.equals(jpvm.pvm_mytid())) {
				...
				try {
					jpvm.pvm_send(creq, target.tid, jpvmDaemonMessageTag.jpvmdCreateTask);
				} catch (jpvmException jpe) {
					perror("SpawnTask, error scheduling " + "on host " + target.tid.toString());
				}
			}
	...
	private static void CreatedTask(jpvmTaskId client, jpvmBuffer req) {
		...
		if (order.numDone == order.num) {
			...
			try {
				jpvm.pvm_send(buf, order.client, jpvmDaemonMessageTag.jpvmdSpawnTask);
			} catch (jpvmException jpe) {
	...
	private static void CreateTask(jpvmTaskId client, jpvmBuffer req) {
		...
		args[6] = name;
		if (debug_on)
			log("exec( " + args[0] + " " + args[1] + " " + args[2] + " " + args[3] + " " + args[4] + " " + args[5] + " )");

		jpvmExecTaskThread spawnThread;
		spawnThread = new jpvmExecTaskThread(jpvm, client, args, createOrders[nextCreateOrder]);
		nextCreateOrder++;
		spawnThread.start();
	}
	...
	private static void TaskStatus(jpvmTaskId client, jpvmBuffer req) {
		...
		try {
			...
			perror("TaskStatus, \"" + jpe + "\" sending to client " + client.toString());
		}
	}
	...
	private static void Ping(jpvmTaskId client, jpvmBuffer req) {
		try {
			...
			perror("ping, \"" + jpe + "\" sending to client " + client.toString());
		}
	}
	...
	private static jpvmBuffer internalAddHosts(jpvmBuffer req) {
		...
		for (i = 0; i < newNum; i++) {
			boolean valid = true;
			try {
				jpvm.pvm_send(pingBuf, newTids[i], jpvmDaemonMessageTag.jpvmdPingRequest);
				jpvmMessage pingMess = jpvm.pvm_recv(newTids[i], jpvmDaemonMessageTag.jpvmdPingReply);
			} catch (jpvmException jpe) {
				valid = false;
				newValidNum--;
	...
	private static void HostStatus(jpvmTaskId client, jpvmBuffer req) {
		...
		try {
			...
			perror("HostStatus, \"" + jpe + "\" sending to client " + client.toString());
		}
	}
