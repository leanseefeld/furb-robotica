...
/**
 * Objeto que armazena a sequência de passos para que o robô se desloque de uma posição até outra.
 */
public class Caminho implements Enumeration<int[]> {
	...
	for (int i = 1; i < caminho.size() - 1; i++) {
	    if (caminho.get(i - 1)[0] == caminho.get(i + 1)[0] && caminho.get(i - 1)[1] == caminho.get(i + 1)[1]) {
		//Remove a ponta
		caminho.remove(i);
