...
public class MapaLabirinto {
	...
	public boolean comparaCooredenadas(int[] a, int[] b) {
		return a[Matriz.COLUNA] == b[Matriz.COLUNA]
				&& a[Matriz.LINHA] == b[Matriz.LINHA];
	}
	 ...
	 * @return Direção da coordenada de destino em relação a coordenada de
	 *         origem
	 */
	...
	public int[][] getVisinhosConexosNaoExplorados(int[] coordenada) {
		...
		if (!this.isIndicesValidos(coordenada)
				|| this.getInfoPosicao(coordenada) == null) {
			return new int[0][2];
		}
	...
	/**
	 * Retorna todos as coordenadas vizinhas conexas (que possuem passagem entre
	 * si) <br>
	 * 
	 * @param coordenada
	 ...
	 *            <span style="color:red">Não funciona se esta celula não foi
	 *            explorada</span>
	 * @param apenasExplorados
	 *            Se true, busca apenas as coordenadas que já foram exploradas
	 * @return vizinhos conexos
	 */
	public int[][] getVizinhos(int[] coordenada, boolean apenasConexos,
			boolean apenasExplorados) {
		List<int[]> vizinhos = new ArrayList<>(4);

		if (!this.isIndicesValidos(coordenada)
				|| (apenasConexos && this.getInfoPosicao(coordenada) == null)) {
			return new int[0][2];
		}
	...
	public boolean isIndicesValidos(int[] coordenada) {
		return coordenada[Matriz.COLUNA] < this.posicoes.length
				&& coordenada[Matriz.LINHA] < this.posicoes[0].length // 
				&& coordenada[Matriz.COLUNA] >= 0
				&& coordenada[Matriz.LINHA] >= 0;
	}

	/**
	 * Busca o menor caminho até uma posição utilizando o algoritmo de
	 * Dijkstra
	 * 
	 * @param origem
	 * @param destino
	...
	private int converterIndexGrafo(int[] coordenada) {
		return converteIndexGrafo(coordenada[Matriz.LINHA],
				coordenada[Matriz.COLUNA]);
	}
}
