...
public class Main {
	...
	System.out.println();
	System.out.println("# # # # # # # # # # # # # # # # # # # # # # # # # # # # # #");
	System.out.println("------- PROCURANDO MENOR CAMINHO");
	System.out.println();

	if (robo.mapeamentoEstaCompleto()) {
	    robo.moveRoboParaPontoInicial();
	    Caminho menorCaminho = robo.montarMenorCaminho();
	    robo.setCaminho(menorCaminho);
	    System.out.println(robo.getCaminho().toString());
	}

	while (true) {
	    if (robo.mapeamentoEstaCompleto() && !robo.getCaminho().isAfterLast()) {
		robo.moverProximaPosicao();
	    } else {
		break;
	    }
